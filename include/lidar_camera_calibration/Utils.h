...
//将激光在相机坐标系下的坐标投影到图像中，新建一个图像，如果这个点投影后在图像中则在这个新建的图像中标注这个点
cv::Mat project(cv::Mat projection_matrix, cv::Rect frame, pcl::PointCloud<pcl::PointXYZ> point_cloud, pcl::PointCloud<pcl::PointXYZ> *visible_points)
{
	...
	cv::Mat plane_gray;
	cv::normalize(plane, plane_gray, 0, 255, cv::NORM_MINMAX, CV_8UC1);//输入的是plane，输出的是plane_grey,plane里面的值不是0就是250
	cv::dilate(plane_gray, plane_gray, cv::Mat());//图像进行膨胀，膨胀就是求局部最大值的操作。为了能够显示的清楚一点
